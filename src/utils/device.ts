...
export function getBootedSimulators(): SimulatorDevice[] {
  ...
  if (json) {
    try {
      ...
      const all: SimulatorDevice[] = Object.entries(data.devices ?? {}).flatMap(
        ([runtime, arr]) =>
          (arr ?? [])
...
export function getAndroidAvds(): string[] {
  ...
  const out = emulatorBin ? safeExec(`"${emulatorBin}" -list-avds`) : safeExec(`emulator -list-avds`);
  if (!out) {return [];}
  return out
    .split(/\r?\n/)
    .map((s) => s.trim())
...
function resolveEmulatorBinary(): string | null {
  const roots = [
    process.env.ANDROID_SDK_ROOT,
    process.env.ANDROID_HOME,
    PLAT === "win32" ? `${os.homedir()}\\AppData\\Local\\Android\\Sdk` : `${os.homedir()}/Library/Android/sdk`,
